...
export class PdfComponent implements OnInit{
  ...
  constructor(private serviceSummaryService:SummaryServiceService,
  ...
  getServicesSummary() {
    this.serviceSummaryService.getTopServices().subscribe({
      next: (data) => {
        ...
        this.porcents = this.serviceSummary.map((summary) => summary.reservationCount);
  ...
  createVelocimeter(): void {
    const ctx = document.getElementById('velocimeterChart') as HTMLCanvasElement;
    new Chart(ctx, {
      type: 'bar',
      data: {
  ...
  createSecondVelocimeter(): void {
    const ctx = document.getElementById('secondVelocimeterChart') as HTMLCanvasElement;
    new Chart(ctx, {
      type: 'bar',
      data: {
        labels: ['Average Score'],
  ...
  generatePDF() {
    ...
    if (reportContainer) {
      html2canvas(reportContainer as HTMLElement, { scale: 2 }).then(canvas => {
        ...
        for (let page = 0; page < totalPages; page++) {
          ...
          pageCanvas.height = Math.min(pageHeightPx, canvas.height - position); // Altura restante o tamaño de la página
          ...
          pdf.addImage(imgData, "PNG", 0, 0, imgWidth, (pageCanvas.height * pdfWidth) / canvas.width); // Añadimos la imagen al PDF
